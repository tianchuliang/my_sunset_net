...
#         return f.name
    if train==True:
        with open('/Users/tianchuliang/documents/gt_acad/7616spring16/hwsoln/hw4/nets/my_sunset_net/my_sunset_net_train_aug_cmdline.prototxt', 'w') as f:
            f.write(str(n.to_proto()))
            return f.name
    else:
        with open('/Users/tianchuliang/documents/gt_acad/7616spring16/hwsoln/hw4/nets/my_sunset_net/my_sunset_net_test_aug_cmdline.prototxt', 'w') as f:
            f.write(str(n.to_proto()))
            return f.name

        
def sunset_net(lmdb, batch_size, train=True, learn_all=False,subset=None,mirror=False,crop_size=256):
    lmdb_mode=dict({'train':'Train_aug','test':'Test'})
    
    if subset is None:
        subset = 'train' if train else 'test'
    string = ""
    if subset is 'train':
        string = "sunset_%s_lmdb_aug" % subset
    else:
        string = "sunset_%s_lmdb" % subset

    lmdb=lmdb+"/"+lmdb_mode[subset]+"/"+string
    ...
    transform_param = dict(mirror = mirror, crop_size=crop_size,
                           mean_file=data_root + '/Sunset/Train_aug/sunset_train_aug_mean.binaryproto')
...
def solver(train_net_path, test_net_path=None, base_lr = 0.001,displayFreq=20,snapshotFreq=100
    ...
    s.snapshot_prefix=nets_root+"/my_sunset_net/my_sunset_net_aug_cmdline"
    s.solver_mode = caffe_pb2.SolverParameter.CPU
    
#     with tempfile.NamedTemporaryFile(delete=False) as f:
#         f.write(str(s))
#         return f.name
    with open('/Users/tianchuliang/documents/gt_acad/7616spring16/hwsoln/hw4/nets/my_sunset_net/my_sunset_net_solver_aug_cmdline.prototxt', 'w') as f:
        f.write(str(s))
        return f.name
